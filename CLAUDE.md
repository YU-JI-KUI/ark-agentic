...
- **Framework, not application.** ark-agentic is a vertical-agnostic agentic foundation — downstream consumers adapt it to finance, insurance, and similar domains. Before coding, ask: is this change a one-off for a single scenario, or a generic capability? Reject hardcoded business rules, domain vocabulary, industry-specific branches, and scenario-coupled logic in framework code. Vertical specifics belong in the consumer's agents/plugins; the framework exposes Protocols and extension points, not opinions.
