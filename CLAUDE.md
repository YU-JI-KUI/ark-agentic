...
- **Framework, not application.** ark-agentic is a vertical-agnostic agentic foundation — downstream consumers adapt it to finance, insurance, and similar domains. Before coding, ask: is this change a one-off for a single scenario, or a generic capability? `core/` and `plugins/` must stay domain-neutral — no hardcoded business rules, industry vocabulary, or scenario-coupled branches; they expose Protocols and extension points, not opinions. Concrete business logic lives only in `agents/`, which is where vertical specifics are allowed.
