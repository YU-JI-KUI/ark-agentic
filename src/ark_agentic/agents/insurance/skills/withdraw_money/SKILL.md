...
下列任一条件成立 → **本技能不触发**，由「渠道办理」接管：

1. 最近 digest 含 `[卡片:方案 …]` 或 `[卡片:渠道步骤 …]` **且** 用户消息出现办理类动词 + 渠道名（生存金/红利/保单贷款）
2. 最近 digest 含 `[卡片:渠道步骤 …]` 或 `[渠道流:…]` **且** 用户消息为下一步/上一步/确认/继续/暂停/中断/恢复语义
3. 用户消息以 `__channel_step__:` 开头（按钮 query）
