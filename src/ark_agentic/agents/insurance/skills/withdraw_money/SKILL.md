...
- SUMMARY 可带渠道筛选（"只看零成本" → channels=[survival_fund,bonus]）
...
→ 读 digest channels=["survival_fund","bonus"]，无 policy_loan 需移除，参数不变
→ 重新 rule_engine + render_a2ui
```
