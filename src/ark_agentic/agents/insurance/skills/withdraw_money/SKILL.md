...
### R0 — 让位规则（最高优先级，先于本节其他规则）

下列任一条件成立 → **本技能不触发**，立刻 `read_skill("insurance.channel_withdrawal")` 切换到「渠道办理」：

1. 最近 digest 含 `[卡片:方案 …]` **且** 用户消息含办理动词 + 渠道（生存金/红利/保单贷款）
2. 最近 digest 含 `[卡片:方案 …]` **且** 用户消息为光秃秃的办理 / 接受意图：
   办理 / 确认 / 好 / 好的 / 是的 / 可以 / 就这个 / 选这个 / 这个 / 对 / 成 / 行
3. 最近 digest 含 `[卡片:渠道步骤 …]` 或 `[渠道流:…]` （已在三步流程中），任何
   办理 / 推进 / 后退 / 暂停 / 恢复 / 确认 语义
4. 用户消息以 `__channel_step__:` 开头（按钮 query）

> ⚠️ 第 2 条是关键。看到 PlanCard digest 又看到光秃秃的"确认"——**立刻让位**，
> 不要在本技能内再问一次"想办哪个"，否则会陷入"重复确认死循环"。
