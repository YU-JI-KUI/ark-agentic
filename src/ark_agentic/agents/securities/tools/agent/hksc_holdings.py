...
**其他可选字段**：
- account_type: 账户类型（可选，HKSC 不区分账户类型，但参数保留以保持一致性）
    ...
    async def execute(
        ...
        if account_type == "margin":
            ...
            return AgentToolResult.json_result(
                ...
                data={"message": "两融账户不支持查询ETF持仓，请调用 render_a2ui 展示提示卡片。"},
                metadata={"state_delta": {self.name: error_data}},
            )

        try:
            # 传递完整 context 给 adapter（用于参数映射和 header 认证）
            data = await create_service_adapter("hksc_holdings", context=context).call(
