        ...
        payload.append(
            {
                ...
                "result_type": getattr(getattr(result, "result_type", None), "value", getattr(result, "result_type", None)),
                "content": getattr(result, "content", None),
                "is_error": getattr(result, "is_error", None),
                "metadata": getattr(result, "metadata", {}),
                "loop_action": getattr(getattr(result, "loop_action", None), "value", getattr(result, "loop_action", None)),
            }
        )
    return payload
...
def create_tracing_callbacks(
    *,
    ...
    """Create Phoenix/OpenTelemetry tracing hooks for the runner lifecycle."""
    ...
    async def _after_agent(ctx: CallbackContext, *, response: Any) -> None:
        ...
        _close_span(
            ...
            attributes={
                ...
                "ark.response_has_tool_calls": bool(getattr(response, "tool_calls", None)),
                "ark.response_content_length": len(getattr(response, "content", "") or ""),
    ...
    async def _before_model(
        ...
        *,
        ...
        handle = _start_managed_span(
            ...
            tracer_name=tracer_name,
            attributes={
                SpanAttributes.OPENINFERENCE_SPAN_KIND: getattr(OpenInferenceSpanKindValues.CHAIN, "value", OpenInferenceSpanKindValues.CHAIN),
                "ark.turn": turn,
    ...
    async def _after_model(ctx: CallbackContext, *, turn: int, response: Any) -> None:
        ...
        usage = response.metadata.get("usage", {}) if getattr(response, "metadata", None) else {}
        _close_span(
            ctx,
            "model",
            attributes={
                "ark.turn": turn,
                "ark.finish_reason": response.metadata.get("finish_reason") if response.metadata else None,
    ...
    async def _after_tool(ctx: CallbackContext, *, turn: int, results: list[Any]) -> None:
        ...
        _close_span(
            ...
            attributes={
                ...
                "ark.error_count": sum(1 for result in results if getattr(result, "is_error", False)),
                "ark.stop_result_count": sum(
                    1
                    for result in results
                    if getattr(getattr(result, "loop_action", None), "value", None) == "stop"
                ),
                **_json_input_output_attributes(output_value=_tool_result_payload(results)),
            },
        )
        return None
