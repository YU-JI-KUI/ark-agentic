...
Jobs currently shares the central ``core.db`` engine — but the feature's
tables live on its own ``JobsBase.metadata``, so ``init_schema()`` here
truly creates only the jobs schema. A future split (dedicated DB,
