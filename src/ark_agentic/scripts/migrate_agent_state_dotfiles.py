...
def migrate_dotfiles(
    ...
    *,
    ...
    """Walk ``memory_dir/{agent_id}/{user_id}/.last_job_<job_id>`` and move
    each dotfile to ``job_runs_dir/{user_id}/.<job_id>``.

    Returns ``(moved, skipped)``. Identical job_id markers across multiple
    agents collapse to one row at the destination — by construction, each
    proactive job has a globally unique ``job_id`` so no real collision
    can happen.
    """
    moved = 0
    skipped = 0
    ...
    if not memory_dir.exists():
        logger.warning("memory_dir %s does not exist; nothing to migrate", memory_dir)
        return 0, 0
...
def main() -> None:
    ...
    parser.add_argument(
        "--memory-dir", type=Path, default=Path("data/ark_memory"),
        help="Source root: data/ark_memory",
    )
    parser.add_argument(
    ...
    moved, skipped = migrate_dotfiles(
        args.memory_dir, args.job_runs_dir, dry_run=args.dry_run,
    )
