...
def _render_history_line(msg: AgentMessage, a2ui_tc_ids: set[str]) -> str:
    ...
    if msg.role == MessageRole.TOOL and msg.tool_results:
        ...
        for tr in msg.tool_results:
            digest = format_tool_result_for_history(tr, a2ui_tc_ids)
            lines.append(f"tool: {digest}")
        return "\n".join(lines)
    return ""


def _build_a2ui_tc_ids(history: list[AgentMessage]) -> set[str]:
    """复刻 runner._build_messages 的 A2UI shadow 检测，确保 router 与主 LLM 一致。"""
    ids: set[str] = set()
    for msg in history:
        if msg.tool_calls:
            for tc in msg.tool_calls:
                if tc.name == "render_a2ui":
                    ids.add(tc.id)
        if msg.tool_results:
            for tr in msg.tool_results:
                if tr.result_type == ToolResultType.A2UI:
                    ids.add(tr.tool_call_id)
    return ids
...
class LLMSkillRouter:
    ...
    def _build_user_prompt(self, ctx: RouteContext) -> str:
        a2ui_tc_ids = _build_a2ui_tc_ids(ctx.history)
        history_window = (
        ...
        for msg in history_window:
            line = _render_history_line(msg, a2ui_tc_ids)
            if line:
