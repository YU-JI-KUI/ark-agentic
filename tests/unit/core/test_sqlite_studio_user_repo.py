...
from ark_agentic.core.storage.repository.sqlite.studio_user import SqliteStudioUserRepository


async def test_ensure_schema_is_public_and_idempotent(tmp_path):
    reset_engine_cache()
    try:
        cfg = DBConfig(db_type="sqlite", connection_str=f"sqlite+aiosqlite:///{tmp_path}/test.db")
