"""Hand-written initial migration for jobs must produce the same schema as
``JobsBase.metadata`` — drift here is a divergence between the migration
file and the ORM model.
"""
...
import ark_agentic.plugins.jobs.storage as _jobs_storage_pkg

_JOBS_MIGRATIONS = Path(_jobs_storage_pkg.__file__).parent / "migrations"
...
async def test_alembic_schema_matches_metadata_create_all() -> None:
    ...
    schema_a.pop("alembic_version_jobs", None)
