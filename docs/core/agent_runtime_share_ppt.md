...
## 第 14 页：reasoning_content 原生思考流

### 建议标题
思考流与最终答案分离：对齐 Thinking 模型的 `reasoning_content` 原生字段

### 核心能力
- 直接消费 OpenAI / vLLM 的 `chunk.additional_kwargs['reasoning_content']`
- `LLMCaller.call_streaming` 检测到后路由到独立的 `thinking_callback`
- 事件流以 `on_thinking_delta` / `thinking_message_*` 独立推送，不污染最终文本
- Instruct 模型无该字段 → 行为零影响

### 价值
- 复用模型侧已训练成熟的思考态输出，不再依赖自定义 `<think>` 标签的不稳定 prompt 工程
- 思考流与最终 `content` 天然分离，前端可独立折叠 / 隐藏
- 落盘消息只保留最终答案，思考内容按需保留在事件流中
...
- `src/ark_agentic/core/stream/events.py`（`thinking_message_*` 事件）
