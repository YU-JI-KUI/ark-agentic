...
F["持久化存储<br/>core/persistence.py"]
G["用户记忆<br/>core/memory/manager.py"]
H["可观测性<br/>observability/*"]
end
A --> C
B --> C
C --> D
...
## 结论
Ark-Agentic 通过多阶段 Docker 构建实现了精简、安全、可重复的镜像交付；应用入口与生命周期管理确保可观测性与资源正确初始化；JSONL 与 MEMORY.md 的纯文件存储降低了运维复杂度；Phoenix/Langfuse 等可观测性能力便于生产监控与问题定位。结合健康检查、并发安全的文件锁与严格的环境变量配置，可在生产环境中实现稳定、可扩展的 Agent 服务交付。
...
- 容器化优势与注意事项
  - 优势：镜像体积小、启动快、可移植、安全基线统一
  - 注意事项：持久化卷、文件锁行为差异、HEALTHCHECK 与探针配置