...
## 更新摘要
**所做更改**
- 移除了 thinking 回调处理机制，简化了流式响应机制
- 更新了 LLM 调用器的流式处理逻辑，不再识别和路由 reasoning_content 字段
- 更新了流式事件总线的 thinking 相关事件处理
- 更新了前端静态页面中对 thinking 事件的处理逻辑
...
**更新** 本版本移除了 thinking 回调处理机制，简化了流式响应机制，提升了系统的简洁性和性能。
...
- 流式事件总线：StreamEventBus（事件分发与生命周期管理）
...
- [event_bus.py:67-84](file://src/ark_agentic/core/stream/event_bus.py#L67-L84)
...
- LLMCaller：LLM 调用封装，支持非流式与流式调用、指数退避重试、流式响应处理。
- ToolExecutor：工具执行器，按序并发执行工具调用，超时与异常兜底，事件统一分发。
- StreamEventBus：流式事件总线，将内部回调转换为 AG-UI 原生事件并推入队列。
...
participant EB as "StreamEventBus"
...
- **更新** 移除了 thinking 回调处理机制，不再识别 Thinking 模型的 reasoning_content 字段。
...
- **更新** 移除了 thinking 相关的事件处理逻辑，简化了事件分发机制。
- 实现 AgentEventHandler 协议，将回调转为 AG-UI 原生 AgentStreamEvent 推入 asyncio.Queue。
- 内部维护 step/text_message 的活跃状态，自动配对 start/finish 事件。
- 支持 on_step、on_content_delta、on_tool_call_*、on_ui_component、on_custom_event 等事件类型。

```mermaid
classDiagram
class StreamEventBus {
+on_step(text : str)
+on_content_delta(delta : str, turn : int)
+on_tool_call_start(tool_call_id : str, name : str, args : dict)
+on_tool_call_result(tool_call_id : str, name : str, result : Any)
+on_ui_component(component : dict[str, Any])
+on_custom_event(custom_type : str, custom_data : dict[str, Any])
-_close_active_step()
-_close_text_message()
...
+on_step(text : str)
+on_content_delta(delta : str, turn : int)
+on_tool_call_start(tool_call_id : str, name : str, args : dict[str, Any])
+on_tool_call_result(tool_call_id : str, name : str, result : Any)
+on_ui_component(component : dict[str, Any])
+on_custom_event(custom_type : str, custom_data : dict[str, Any])
}
StreamEventBus ..|> AgentEventHandler
```
...
- **更新** 简化后的流式输出：支持 on_content_delta、on_tool_call_*、on_ui_component、on_step 等事件。
...
- [test_event_bus.py:149-187](file://tests/unit/core/test_event_bus.py#L149-L187)
...
- 组件耦合
  ...
  - **更新** StreamEventBus 实现 AgentEventHandler 协议，提供事件分发功能。
- 外部依赖
...
SM --> SS["SessionStore"]
EB["StreamEventBus"] --> AH["AgentEventHandler"]
```
...
## 性能考量
- 流式输出：开启流式可降低首屏延迟，提升用户体验；**更新** 简化后的流式模式移除了 thinking 回调处理，减少了不必要的事件分发开销。
...
- **更新** Thinking 事件处理
  - **移除** thinking 相关的回调处理，如需显示思考过程，可通过 on_step 事件实现类似功能。
...
## 结论
AgentRunner 通过清晰的生命周期与回调机制，实现了可插拔、可观测、可扩展的 ReAct 执行框架。**更新** 移除 thinking 回调处理后，系统变得更加简洁高效，同时保持了完整的流式响应能力。结合 LLMCaller、ToolExecutor、SessionManager 与 SystemPromptBuilder，系统在保证稳定性的同时提供了强大的定制能力。合理配置 RunnerConfig 与 RunOptions，配合回调钩子与事件总线，可满足复杂业务场景下的智能体执行需求。
...
- 关键 API 与路径
  ...
  - StreamEventBus.on_step/on_content_delta/on_tool_call_*等事件处理
