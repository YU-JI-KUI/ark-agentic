...
- [src/ark_agentic/agents/securities/tools/service/stock_search/index.py](file://src/ark_agentic/agents/securities/tools/service/stock_search/index.py)
- [src/ark_agentic/agents/securities/tools/service/stock_search/loader.py](file://src/ark_agentic/agents/securities/tools/service/stock_search/loader.py)
- [src/ark_agentic/agents/securities/tools/service/stock_search/matcher.py](file://src/ark_agentic/agents/securities/tools/service/stock_search/matcher.py)
...
- [src/ark_agentic/agents/securities/README.md](file://src/ark_agentic/agents/securities/README.md)
- [pyproject.toml](file://pyproject.toml)
- [tests/unit/agents/securities/test_security_info_search.py](file://tests/unit/agents/securities/test_security_info_search.py)
- [src/ark_agentic/agents/securities/mock_data/stocks/a_shares_seed.csv](file://src/ark_agentic/agents/securities/mock_data/stocks/a_shares_seed.csv)
- [src/ark_agentic/agents/securities/mock_data/dividends/default.json](file://src/ark_agentic/agents/securities/mock_data/dividends/default.json)
</cite>

## 更新摘要
**变更内容**
- 新增了完整的股票搜索服务架构说明，包括 StockSearchService、StockLoader、MultiPathMatcher 等核心组件
- 补充了工具集成模式、API通信协议、服务适配器依赖关系、性能优化措施和股票搜索服务的详细实现说明
- 更新了工具组组织方式和依赖关系，从 '[stock-search]' 迁移到 '--group securities'
...
**更新** 本次更新重点反映了股票搜索功能从 '[stock-search]' 到 '--group securities' 的重大迁移，新增了完整的股票搜索服务架构和实现细节，包括进程内单例 Loader、LRU 缓存、向量化批量匹配等性能优化措施。
...
SSS["StockSearchService<br/>股票搜索服务"]
end
subgraph "数据模型层"
SCH["schemas.py<br/>Pydantic 模型"]
SMS["stock_search/models.py<br/>搜索数据模型"]
end
AO --> CSA
AP --> CSA
BI --> CSA
...
**更新** 新增了股票搜索服务组件，包括 StockSearchService、StockLoader、MultiPathMatcher 等核心组件，以及完整的依赖关系和性能优化措施。
...
### 股票信息搜索工具
- 功能用途
  - 提供股票代码/名称的检索能力，辅助用户定位目标标的。支持精确代码匹配、中文名称模糊匹配、拼音/首字母匹配和错别字纠错。
- 参数定义
  - query：6 位股票代码或股票名称/拼音，如 "600519"、"贵州茅台"、"maotai"
  - include_dividend：是否返回分红信息，默认 false
- 返回数据结构
  - 包含匹配状态、置信度、分数、股票实体、分红信息和候选列表
  - confidence 取值：exact（精确匹配）、high（高置信度）、ambiguous（模糊匹配）、none（未匹配）
- 调用方式
  - 通过 StockSearchService 执行搜索，支持进程内缓存的股票查询能力
- 使用场景
  - 用户输入模糊名称或代码片段时，先搜索再查看详情
  - 支持 ASR 识别偏差处理和用户记忆不准确的情况
- 最佳实践
  - 与股票详情工具串联，形成"搜索-查看"闭环
  - 根据置信度选择合适的后续操作：exact 直接查看详情，ambiguous 展示候选供确认

**更新** 重新实现了股票搜索功能，从之前的 '[stock-search]' 迁移到 '--group securities'，采用全新的 StockSearchService 架构，包括进程内单例 Loader、LRU 缓存、向量化批量匹配等性能优化措施。
...
- [src/ark_agentic/agents/securities/tools/service/stock_search_service.py:32-84](file://src/ark_agentic/agents/securities/tools/service/stock_search_service.py#L32-L84)
- [src/ark_agentic/agents/securities/tools/service/stock_search/index.py:53-148](file://src/ark_agentic/agents/securities/tools/service/stock_search/index.py#L53-L148)
- [src/ark_agentic/agents/securities/tools/service/stock_search/loader.py:74-138](file://src/ark_agentic/agents/securities/tools/service/stock_search/loader.py#L74-L138)
- [src/ark_agentic/agents/securities/tools/service/stock_search/matcher.py:59-239](file://src/ark_agentic/agents/securities/tools/service/stock_search/matcher.py#L59-L239)
- [src/ark_agentic/agents/securities/tools/service/stock_search/models.py:114-136](file://src/ark_agentic/agents/securities/tools/service/stock_search/models.py#L114-L136)
...
**更新** 新增了股票搜索服务的依赖关系，包括 StockSearchService、StockLoader、MultiPathMatcher 等组件的协作关系，以及可选依赖 securities 组的安装要求。
...
A --> O["输出数据"]
C["Agent 回调"] --> T
SSS["StockSearchService"] --> SL["StockLoader"]
SSS --> MP["MultiPathMatcher"]
SL --> SI["StockIndex"]
MP --> SI
SSS --> D
```
...
**更新** 新增了股票搜索服务的性能优化措施，包括进程内单例 Loader、LRU 缓存、向量化批量匹配等。

- 股票搜索服务性能优化
  - 进程内单例 StockLoader：首次调用时初始化，避免重复 IO 开销
  - LRU 缓存：StockIndex 和 Mock 分红数据采用缓存机制
  - 向量化批量匹配：使用 rapidfuzz 的向量化接口提升模糊匹配性能
  - 多路匹配流水线：精确代码匹配优先，拼音/首字母匹配降级处理
  - 可选依赖管理：通过 securities 依赖组提供可选的 akshare、rapidfuzz、pypinyin 支持

**章节来源**
- [pyproject.toml:95-99](file://pyproject.toml#L95-L99)
...
- [src/ark_agentic/agents/securities/tools/service/stock_search/loader.py:52-71](file://src/ark_agentic/agents/securities/tools/service/stock_search/loader.py#L52-L71)
- [src/ark_agentic/agents/securities/tools/service/stock_search/matcher.py:35-42](file://src/ark_agentic/agents/securities/tools/service/stock_search/matcher.py#L35-L42)
...
**更新** 新增了股票搜索功能的故障排查指南，包括可选依赖安装和环境配置。

- 股票搜索功能故障排查
  - 现象：rapidfuzz 未安装导致功能降级
  - 处理：安装依赖 `uv add rapidfuzz pypinyin` 或使用 `uv sync --group securities`
  - 现象：股票数据为空或查询不到结果
  - 处理：检查 STOCKS_CSV_PATH 环境变量是否指向有效 CSV 文件
  - 现象：拼音匹配功能不可用
  - 处理：安装 pypinyin 依赖，或使用代码精确匹配和名称完全匹配
  - 现象：置信度过低返回 ambiguous
  - 处理：提供更精确的查询条件或查看候选列表
  - 现象：Mock 分红数据未返回
  - 处理：检查 SECURITIES_SERVICE_MOCK 环境变量或上下文 mock_mode 设置
...
- [tests/unit/agents/securities/test_security_info_search.py:18-24](file://tests/unit/agents/securities/test_security_info_search.py#L18-L24)
...
**更新** 本次更新重点强化了股票搜索功能的实现，从 '[stock-search]' 迁移到 '--group securities'，采用了全新的 StockSearchService 架构，提供了精确代码匹配、中文名称模糊匹配、拼音/首字母匹配和错别字纠错等强大功能，显著提升了用户体验和查询准确性。新增的性能优化措施包括进程内单例缓存、LRU 缓存和向量化批量匹配，确保了高效稳定的查询体验。
...
- 工具清单与职责
  ...
  - 港股通持仓：港股通额度、持仓与预冻结。
  - 股票详情：单只股票持仓与行情。
  - 股票搜索：标的检索辅助，支持多种匹配方式。
  - 月度日收益：按月度的日收益序列。
  - 日收益范围：按日期范围的日收益序列。
  - 收益排名：股票池内收益排名。

**更新** 新增了股票搜索工具的详细职责说明，包括支持的匹配方式和使用场景，以及可选依赖组 securities 的安装要求。

**章节来源**
- [pyproject.toml:95-99](file://pyproject.toml#L95-L99)
- [src/ark_agentic/agents/securities/mock_data/stocks/a_shares_seed.csv:1-800](file://src/ark_agentic/agents/securities/mock_data/stocks/a_shares_seed.csv#L1-L800)
- [src/ark_agentic/agents/securities/mock_data/dividends/default.json:1-63](file://src/ark_agentic/agents/securities/mock_data/dividends/default.json#L1-L63)