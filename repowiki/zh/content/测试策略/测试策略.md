# 测试策略

<cite>
**本文档引用的文件**
- [tests/conftest.py](file://tests/conftest.py)
- [pyproject.toml](file://pyproject.toml)
- [README.md](file://README.md)
- [tests/unit/core/test_runner.py](file://tests/unit/core/test_runner.py)
- [tests/integration/test_agent_integration.py](file://tests/integration/test_agent_integration.py)
- [tests/e2e/test_memory_e2e.py](file://tests/e2e/test_memory_e2e.py)
- [tests/unit/agents/insurance/test_withdrawal_multiturn.py](file://tests/unit/agents/insurance/test_withdrawal_multiturn.py)
- [tests/integration/agents/securities/test_mock_loader_and_service_adapter.py](file://tests/integration/agents/securities/test_mock_loader_and_service_adapter.py)
- [scripts/test_subtask_e2e.py](file://scripts/test_subtask_e2e.py)
- [src/ark_agentic/agents/insurance/tools/data_service.py](file://src/ark_agentic/agents/insurance/tools/data_service.py)
- [src/ark_agentic/agents/securities/tools/service/mock_loader.py](file://src/ark_agentic/agents/securities/tools/service/mock_loader.py)
- [scripts/update_mock_data_to_str.py](file://scripts/update_mock_data_to_str.py)
- [scripts/gen_seed_data.py](file://scripts/gen_seed_data.py)
- [src/ark_agentic/app.py](file://src/ark_agentic/app.py)
- [src/ark_agentic/static/home.html](file://src/ark_agentic/static/home.html)
</cite>

## 更新摘要
**所做更改**
- 新增维基系统章节，详细介绍repowiki目录结构和维基文档管理系统
- 更新测试框架章节，增加维基系统测试相关内容
- 扩展测试工具使用指南，包含维基文档访问和测试方法
- 新增维基系统测试策略和质量保证流程

## 目录
1. [引言](#引言)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [维基系统测试策略](#维基系统测试策略)
7. [依赖分析](#依赖分析)
8. [性能考虑](#性能考虑)
9. [故障排查指南](#故障排查指南)
10. [结论](#结论)
11. [附录](#附录)

## 引言
本测试策略文档面向 Ark-Agentic 项目，系统阐述测试框架、单元测试、集成测试、端到端测试与性能测试的实施方法，涵盖测试用例设计、Mock 数据管理、回归测试与持续集成实践，以及测试工具使用指南、测试覆盖率要求与质量保证流程。随着新增完整维基系统的引入，文档现包含维基文档管理、测试自动化和质量保证的综合测试策略，目标是帮助开发者在不同测试层级高效定位问题、降低风险并提升交付质量。

## 项目结构
测试相关目录组织遵循"按层级与功能划分"的原则，现已扩展包含维基系统测试：
- tests/unit：核心模块与工具的单元测试，强调隔离与可重复性
- tests/integration：跨模块/跨组件的集成测试，验证真实依赖与外部服务适配
- tests/e2e：端到端测试，覆盖真实业务闭环与长生命周期行为
- tests/skills：技能评估与基准测试工作区，包含静态评估与迭代评测
- scripts：辅助测试脚本，如子任务 E2E、Mock 数据转换与种子数据生成
- repowiki：维基文档系统，包含完整的中英文文档结构

```mermaid
graph TB
A["tests/"] --> B["unit/"]
A --> C["integration/"]
A --> D["e2e/"]
A --> E["skills/"]
F["scripts/"] --> G["test_subtask_e2e.py"]
F --> H["update_mock_data_to_str.py"]
F --> I["gen_seed_data.py"]
B --> B1["core/*"]
B --> B2["agents/*"]
C --> C1["agents/securities/*"]
C --> C2["agents/insurance/*"]
D --> D1["test_memory_e2e.py"]
J["repowiki/"] --> J1["zh/content/"]
J --> J2["en/content/"]
J1 --> J3["测试策略.md"]
J1 --> J4["测试框架.md"]
J2 --> J5["Test Strategy.md"]
J2 --> J6["Test Framework.md"]
```

**图表来源**
- [src/ark_agentic/app.py:172-173](file://src/ark_agentic/app.py#L172-L173)
- [src/ark_agentic/app.py:198-246](file://src/ark_agentic/app.py#L198-L246)
- [src/ark_agentic/static/home.html:1200-1315](file://src/ark_agentic/static/home.html#L1200-L1315)

**章节来源**
- [README.md:757-770](file://README.md#L757-L770)

## 核心组件
- 测试框架与配置
  - 使用 pytest，启用 asyncio 自动模式，设置测试路径与超时，标记"慢测试"
  - 通过 conftest 注入 src 路径与可选依赖的 mock，确保在缺失依赖时仍可运行
- 测试用例组织
  - unit：最小可执行单元，依赖 Mock/LangChain 兼容接口
  - integration：真实外部服务适配器与工具链集成
  - e2e：长生命周期与状态注入场景
  - skills：技能静态评估与迭代评测
- Mock 数据与环境
  - 保险/证券模块均提供 Mock 客户端与加载器，支持多场景数据
  - 提供脚本进行 Mock 数据清洗与种子数据生成
- 维基系统测试
  - 支持中英文双语维基文档的自动化测试
  - 包含目录树构建、页面加载和导航测试
  - 集成元数据驱动的内容排序和权限控制测试

**章节来源**
- [pyproject.toml:70-78](file://pyproject.toml#L70-L78)
- [tests/conftest.py:1-39](file://tests/conftest.py#L1-L39)
- [README.md:757-770](file://README.md#L757-L770)

## 架构总览
测试架构围绕"隔离 Mock + 真实集成 + 端到端闭环 + 维基系统"展开，结合环境变量与单例客户端实现灵活的 Mock/真实切换，同时支持维基文档的自动化测试。

```mermaid
graph TB
subgraph "测试层"
UT["单元测试<br/>tests/unit/*"]
IT["集成测试<br/>tests/integration/*"]
ET["端到端测试<br/>tests/e2e/*"]
SK["技能评估<br/>tests/skills/*"]
WS["维基系统测试<br/>repowiki/*"]
end
subgraph "业务模块"
INS["保险模块<br/>agents/insurance/*"]
SEC["证券模块<br/>agents/securities/*"]
API["API 层<br/>app.py"]
HOME["前端界面<br/>home.html"]
end
subgraph "基础设施"
MOCK["Mock 客户端/加载器<br/>data_service.py / mock_loader.py"]
ENV["环境变量<br/>DATA_SERVICE_MOCK / SECURITIES_SERVICE_MOCK"]
SCRIPTS["测试脚本<br/>scripts/*"]
META["元数据<br/>repowiki-metadata.json"]
end
UT --> INS
UT --> SEC
IT --> INS
IT --> SEC
ET --> INS
ET --> SEC
SK --> INS
SK --> SEC
WS --> META
API --> WS
HOME --> API
INS --> MOCK
SEC --> MOCK
ENV --> MOCK
SCRIPTS --> UT
SCRIPTS --> IT
SCRIPTS --> ET
```

**图表来源**
- [src/ark_agentic/app.py:198-261](file://src/ark_agentic/app.py#L198-L261)
- [src/ark_agentic/static/home.html:1200-1315](file://src/ark_agentic/static/home.html#L1200-L1315)
- [src/ark_agentic/agents/insurance/tools/data_service.py:433-445](file://src/ark_agentic/agents/insurance/tools/data_service.py#L433-L445)
- [src/ark_agentic/agents/securities/tools/service/mock_loader.py:1-123](file://src/ark_agentic/agents/securities/tools/service/mock_loader.py#L1-L123)

## 详细组件分析

### 单元测试：AgentRunner 与核心流程
- 设计要点
  - 使用 LangChain 兼容的 Mock LLM 与工具注册表，构造最小可运行环境
  - 验证文本响应、工具调用、流式输出、回调钩子注入等关键路径
  - 通过临时会话目录隔离状态，避免跨用例污染
- 关键断言
  - 响应内容、轮次计数、工具调用次数、会话历史中的工具消息与结果
- 可扩展性
  - RunnerCallbacks 注入可观测性与质量控制钩子，便于在单元层验证

```mermaid
sequenceDiagram
participant T as "测试用例"
participant R as "AgentRunner"
participant L as "Mock LLM"
participant TR as "ToolRegistry"
participant SM as "SessionManager"
T->>R : "创建 RunnerMock LLM + 工具注册表"
T->>SM : "创建会话"
T->>R : "run(session_id, user_input)"
R->>L : "ainvoke(messages)"
L-->>R : "AIMessage文本/工具调用"
alt "存在工具调用"
R->>TR : "批量执行工具"
TR-->>R : "工具结果"
R->>L : "二次 ainvoke汇总"
L-->>R : "最终 AIMessage"
end
R-->>T : "RunResult内容/轮次/工具计数"
```

**图表来源**
- [tests/unit/core/test_runner.py:141-200](file://tests/unit/core/test_runner.py#L141-L200)
- [tests/unit/core/test_runner.py:197-200](file://tests/unit/core/test_runner.py#L197-L200)

**章节来源**
- [tests/unit/core/test_runner.py:1-200](file://tests/unit/core/test_runner.py#L1-L200)

### 集成测试：保险与证券工具链
- 保险模块
  - 多轮对话与 A2UI 渲染链路，验证规则引擎、计划卡渲染、提交取款、状态流转
  - 通过真实工具与状态合并模拟完整业务闭环
- 证券模块
  - Mock 加载器与服务适配器，覆盖账户概览、ETF 持仓、安全明细等多场景
  - 支持慢测试标记，便于在 CI 中选择性跳过

```mermaid
flowchart TD
Start(["开始：多轮对话"]) --> RuleEngine["规则引擎计算"]
RuleEngine --> RenderPlan["渲染取款计划卡A2UI"]
RenderPlan --> Submit1["提交首笔渠道如生存金"]
Submit1 --> Continue{"是否仍有待办？"}
Continue --> |是| Submit2["提交次笔渠道如红利"]
Continue --> |否| Adjust["进入调整模式重置已提交渠道"]
Submit2 --> Done(["完成：全部渠道处理完毕"])
Adjust --> SubmitLoan["提交保单贷款新计划"]
SubmitLoan --> Done
```

**图表来源**
- [tests/unit/agents/insurance/test_withdrawal_multiturn.py:91-179](file://tests/unit/agents/insurance/test_withdrawal_multiturn.py#L91-L179)

**章节来源**
- [tests/unit/agents/insurance/test_withdrawal_multiturn.py:1-200](file://tests/unit/agents/insurance/test_withdrawal_multiturn.py#L1-L200)
- [tests/integration/agents/securities/test_mock_loader_and_service_adapter.py:1-45](file://tests/integration/agents/securities/test_mock_loader_and_service_adapter.py#L1-L45)

### 端到端测试：记忆系统生命周期
- 覆盖点
  - 上下文压缩触发内存刷新，写入 MEMORY.md
  - MEMORY.md 内容被注入到系统提示词，贯穿后续对话
- 方法论
  - 使用临时目录隔离内存与会话数据
  - 通过 monkeypatch 替换 LLM 调用与 MemoryFlusher.flush，确保可控与可重复

```mermaid
sequenceDiagram
participant E as "E2E 测试"
participant S as "SessionManager"
participant M as "MemoryManager"
participant F as "MemoryFlusher"
participant L as "LLMCaller"
E->>S : "创建会话并多次 run"
S->>S : "达到压缩阈值"
S->>F : "flush(conversation_text, current_memory)"
F->>L : "调用 LLM 摘要"
L-->>F : "摘要结果"
F-->>M : "写入 MEMORY.md"
E->>S : "后续 run"
S->>M : "读取 MEMORY.md"
M-->>S : "注入系统提示词"
```

**图表来源**
- [tests/e2e/test_memory_e2e.py:100-162](file://tests/e2e/test_memory_e2e.py#L100-L162)
- [tests/e2e/test_memory_e2e.py:164-200](file://tests/e2e/test_memory_e2e.py#L164-L200)

**章节来源**
- [tests/e2e/test_memory_e2e.py:1-200](file://tests/e2e/test_memory_e2e.py#L1-L200)

### 子任务并行 E2E（脚本）
- 目标
  - 验证 SpawnSubtasksTool 在真实 LLM 与 Mock 数据服务下的并行执行能力
- 方法
  - 通过脚本直接构造工具调用并执行，绕过 LLM 的触发判断
  - 收集子任务结果、转录与状态增量，便于人工核验

```mermaid
sequenceDiagram
participant Script as "测试脚本"
participant Agent as "Insurance Agent"
participant ST as "SpawnSubtasksTool"
participant LLM as "LLM"
participant DS as "Mock 数据服务"
Script->>Agent : "创建会话"
Script->>ST : "execute(spawn_subtasks)"
ST->>LLM : "为每个子任务生成规划"
ST->>DS : "并行调用数据服务"
DS-->>ST : "返回子任务结果"
ST-->>Script : "汇总结果与转录"
```

**图表来源**
- [scripts/test_subtask_e2e.py:38-93](file://scripts/test_subtask_e2e.py#L38-L93)

**章节来源**
- [scripts/test_subtask_e2e.py:1-93](file://scripts/test_subtask_e2e.py#L1-L93)

### Mock 数据管理与环境切换
- 保险模块
  - 通过环境变量控制 Mock/真实客户端单例，支持测试中动态切换
- 证券模块
  - MockDataLoader 从 JSON 文件加载多场景数据，支持场景枚举与错误兜底
  - 提供脚本将数值型字段统一为字符串，确保前后端一致
  - 提供脚本生成种子 CSV，支撑股票搜索与匹配服务

```mermaid
flowchart TD
Env["设置环境变量<br/>DATA_SERVICE_MOCK / SECURITIES_SERVICE_MOCK"] --> Client["获取客户端单例"]
Client --> IsMock{"Mock 模式？"}
IsMock --> |是| MockClient["MockDataServiceClient"]
IsMock --> |否| RealClient["DataServiceClient"]
MockClient --> API["工具调用Mock 数据"]
RealClient --> API
API --> Result["返回结构化数据"]
subgraph "证券 Mock 数据"
Loader["MockDataLoader.load()"] --> File["读取 JSON 场景文件"]
File --> Result2["返回场景数据"]
Clean["update_mock_data_to_str.py"] --> Loader
Seed["gen_seed_data.py"] --> File
end
```

**图表来源**
- [src/ark_agentic/agents/insurance/tools/data_service.py:433-445](file://src/ark_agentic/agents/insurance/tools/data_service.py#L433-L445)
- [src/ark_agentic/agents/securities/tools/service/mock_loader.py:1-123](file://src/ark_agentic/agents/securities/tools/service/mock_loader.py#L1-L123)
- [scripts/update_mock_data_to_str.py:1-34](file://scripts/update_mock_data_to_str.py#L1-L34)
- [scripts/gen_seed_data.py:1115-1148](file://scripts/gen_seed_data.py#L1115-L1148)

**章节来源**
- [src/ark_agentic/agents/insurance/tools/data_service.py:433-445](file://src/ark_agentic/agents/insurance/tools/data_service.py#L433-L445)
- [src/ark_agentic/agents/securities/tools/service/mock_loader.py:1-123](file://src/ark_agentic/agents/securities/tools/service/mock_loader.py#L1-L123)
- [scripts/update_mock_data_to_str.py:1-34](file://scripts/update_mock_data_to_str.py#L1-L34)
- [scripts/gen_seed_data.py:1115-1148](file://scripts/gen_seed_data.py#L1115-L1148)

## 维基系统测试策略

### 维基系统架构
Ark-Agentic 现已集成完整的维基文档系统，支持中英文双语文档管理：

- **目录结构**
  - zh/content：中文文档内容
  - en/content：英文文档内容
  - zh/meta：中文元数据配置
  - en/meta：英文元数据配置

- **核心功能**
  - 目录树构建与排序
  - 文档内容加载与渲染
  - 语言切换与导航
  - 元数据驱动的内容管理

```mermaid
flowchart TD
subgraph "维基系统架构"
REP["repowiki/"] --> ZH["zh/"]
REP --> EN["en/"]
ZH --> ZH_CONTENT["content/"]
ZH --> ZH_META["meta/"]
EN --> EN_CONTENT["content/"]
EN --> EN_META["meta/"]
ZH_CONTENT --> DOC_ZH["中文文档"]
EN_CONTENT --> DOC_EN["英文文档"]
ZH_META --> META_ZH["中文元数据"]
EN_META --> META_EN["英文元数据"]
end
```

**图表来源**
- [src/ark_agentic/app.py:198-246](file://src/ark_agentic/app.py#L198-L246)

### 维基系统测试实现
- **API 接口测试**
  - `/api/wiki/tree`：目录树构建测试
  - `/api/wiki/{lang}/{path}`：文档内容加载测试
  - 元数据解析与排序测试

- **前端交互测试**
  - 目录树渲染测试
  - 文档内容加载测试
  - 语言切换测试
  - 导航与面包屑测试

- **安全性测试**
  - 路径遍历防护测试
  - 权限控制测试
  - 输入验证测试

```mermaid
sequenceDiagram
participant Test as "维基测试"
participant API as "Wiki API"
participant FS as "文件系统"
participant Meta as "元数据解析"
Test->>API : "GET /api/wiki/tree"
API->>Meta : "load_order_map(lang)"
Meta->>FS : "读取 repowiki-metadata.json"
FS-->>Meta : "返回元数据"
Meta-->>API : "构建排序映射"
API->>FS : "遍历 content 目录"
FS-->>API : "返回目录树结构"
API-->>Test : "JSON 响应"
```

**图表来源**
- [src/ark_agentic/app.py:204-246](file://src/ark_agentic/app.py#L204-L246)

### 维基系统测试用例设计
- **目录树构建测试**
  - 验证中英文目录树正确构建
  - 测试元数据驱动的排序功能
  - 验证目录嵌套结构的正确性

- **文档加载测试**
  - 测试有效文档的加载功能
  - 验证无效路径的错误处理
  - 测试路径遍历攻击防护

- **语言切换测试**
  - 验证中英文内容的正确加载
  - 测试语言切换的响应性
  - 验证语言特定的元数据应用

**章节来源**
- [src/ark_agentic/app.py:198-261](file://src/ark_agentic/app.py#L198-L261)
- [src/ark_agentic/static/home.html:1200-1315](file://src/ark_agentic/static/home.html#L1200-L1315)

## 依赖分析
- 测试框架与工具
  - pytest、pytest-asyncio、pytest-timeout、pytest markers
- 可选依赖的 Mock
  - 在 conftest 中对 sentence_transformers、jieba、torch、numpy 等进行 mock，确保在缺少依赖时测试仍可运行
- 项目依赖与可选组
  - dev 组包含测试工具；pa-jt 组包含加密依赖；all 组聚合两者
- 维基系统依赖
  - FastAPI 用于 API 接口
  - 前端 JavaScript 用于文档渲染
  - Markdown 解析器用于内容展示

```mermaid
graph TB
P["pyproject.toml"] --> Dev["dev 组：pytest, pytest-asyncio, pytest-timeout"]
P --> Optional["optional-dependencies"]
Optional --> DevGroup["ark-agentic[dev]"]
Optional --> PJT["ark-agentic[pa-jt]"]
Optional --> All["ark-agentic[all]"]
C["tests/conftest.py"] --> Mocks["Mock 可选依赖unittest.mock.MagicMock"]
WS["维基系统"] --> FastAPI["FastAPI"]
WS --> JS["JavaScript 前端"]
WS --> MD["Markdown 解析"]
```

**图表来源**
- [pyproject.toml:26-43](file://pyproject.toml#L26-L43)
- [tests/conftest.py:17-30](file://tests/conftest.py#L17-L30)

**章节来源**
- [pyproject.toml:26-43](file://pyproject.toml#L26-L43)
- [tests/conftest.py:17-30](file://tests/conftest.py#L17-L30)

## 性能考虑
- 测试超时与门禁
  - pytest 设置全局 timeout，避免慢测试拖垮全量执行；慢测试使用标记在 CI 中选择性运行
- 并行与流式
  - 单元测试中使用异步 Mock 与流式接口，减少外部依赖带来的阻塞
- 集成测试的 Mock 切换
  - 通过环境变量与单例客户端快速切换 Mock/真实，平衡稳定性与真实性
- 维基系统性能优化
  - 目录树构建使用缓存机制
  - 文档内容采用懒加载策略
  - 元数据解析结果进行本地缓存

**章节来源**
- [pyproject.toml:70-78](file://pyproject.toml#L70-L78)
- [tests/unit/core/test_runner.py:197-200](file://tests/unit/core/test_runner.py#L197-L200)

## 故障排查指南
- 常见问题
  - 缺少可选依赖导致测试失败：确认 conftest 的 mock 注入是否生效
  - Mock/真实模式切换异常：检查环境变量与客户端单例重置逻辑
  - 集成测试超时：为慢测试添加标记并在 CI 中排除
  - 维基系统文档加载失败：检查 repowiki 目录结构和元数据文件
- 定位手段
  - 使用临时目录隔离会话与内存数据，避免状态污染
  - 通过 monkeypatch 替换关键组件（如 LLMCaller、MemoryFlusher），确保可控
  - 使用脚本化 E2E 测试快速验证子任务并行与工具链连通性
  - 验证维基系统 API 端点的响应状态和内容格式

**章节来源**
- [tests/conftest.py:17-30](file://tests/conftest.py#L17-L30)
- [src/ark_agentic/agents/insurance/tools/data_service.py:448-451](file://src/ark_agentic/agents/insurance/tools/data_service.py#L448-L451)
- [tests/e2e/test_memory_e2e.py:100-162](file://tests/e2e/test_memory_e2e.py#L100-L162)
- [scripts/test_subtask_e2e.py:38-93](file://scripts/test_subtask_e2e.py#L38-L93)

## 结论
Ark-Agentic 的测试策略以"单元隔离、集成真实、端到端闭环、技能评估、维基系统"为主线，辅以完善的 Mock 数据管理与环境切换机制。随着维基系统的引入，测试策略现在涵盖了完整的文档管理系统，包括中英文双语支持、元数据驱动的内容管理和安全的文档访问控制。通过 pytest 标记与超时控制，既能保证快速反馈，又能覆盖复杂业务场景和文档系统的完整性。建议在持续集成中区分快慢测试，结合脚本化 E2E 与技能评估报告，形成完整的质量保障闭环。

## 附录

### 测试工具使用指南
- 运行方式
  - 全量测试：使用项目提供的命令入口
  - 指定模块：针对核心模块或特定功能运行单元测试
  - 集成测试：按需开启 LLM 集成（需相应环境变量）
  - 维基系统测试：通过 API 端点验证文档加载功能
- 常用命令
  - 运行全量测试
  - 运行指定单元测试文件
  - 运行集成测试（需设置相应环境变量）
  - 验证维基系统 API：`curl http://localhost:8080/api/wiki/tree`

**章节来源**
- [README.md:757-770](file://README.md#L757-L770)

### 测试覆盖率与质量保证
- 覆盖率要求
  - 建议核心模块（Runner、Session、Memory、A2UI、工具系统）覆盖率不低于 80%
  - 业务智能体与技能链路不低于 70%
  - 维基系统 API 功能不低于 90%
- 质量门禁
  - 通过 pytest 标记区分慢测试，CI 中默认跳过慢测试
  - 使用临时目录与 monkeypatch 保证测试稳定性与可重复性
  - 维基系统测试包含 API 接口验证和前端交互测试
- 维基系统质量保证
  - 目录树构建准确性验证
  - 文档内容完整性检查
  - 多语言支持一致性测试
  - 安全性与性能监控

**章节来源**
- [pyproject.toml:70-78](file://pyproject.toml#L70-L78)
- [tests/e2e/test_memory_e2e.py:43-56](file://tests/e2e/test_memory_e2e.py#L43-L56)
- [src/ark_agentic/app.py:198-261](file://src/ark_agentic/app.py#L198-L261)