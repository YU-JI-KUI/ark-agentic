...
## 更新摘要
**所做更改**   
- 新增 TaskFlow 流程编排框架章节，涵盖 BaseFlowEvaluator、StageDefinition、TaskRegistry 等核心组件
- 更新服务层重构章节，详细介绍 Studio 服务层的三层架构设计
- 新增前端现代化章节，说明 React 19 前端框架和现代化开发工具链
- 更新架构总览图，反映 TaskFlow 框架和 Studio 服务层的集成
- 新增子任务并行执行工具章节，说明 SpawnSubtasksTool 的设计原理
- 更新依赖关系分析，体现新的组件交互模式
...
## 简介
本文件面向 Ark-Agentic 的整体架构设计，聚焦于 ReAct 执行引擎、工具系统、技能系统、A2UI 渲染系统、会话管理、TaskFlow 流程编排框架和 Studio 服务层的协作机制。文档从系统边界、数据流、控制流与扩展性角度出发，结合代码级可视化图表，帮助读者快速理解并高效扩展该架构。

**更新** 本次更新重点反映了 TaskFlow 框架的重大引入、服务层的深度重构以及前端的现代化升级，确保架构文档与当前实现保持完全一致。
...
- [src/ark_agentic/studio/services/agent_service.py:60-198](file://src/ark_agentic/studio/services/agent_service.py#L60-198)
- [src/ark_agentic/studio/services/skill_service.py:44-294](file://src/ark_agentic/studio/services/skill_service.py#L44-294)
- [src/ark_agentic/studio/services/tool_service.py:61-239](file://src/ark_agentic/studio/services/tool_service.py#L61-239)
...
- [src/ark_agentic/core/flow/task_registry.py:32-124](file://src/ark_agentic/core/flow/task_registry.py#L32-124)
...
## 架构总览
Ark-Agentic 采用"多 Agent + 统一 Runner + TaskFlow 编排"的架构模式，API 层负责请求接入与事件流式输出，Runner 层负责 ReAct 循环与工具编排，TaskFlow 框架提供确定性的流程状态管理，底层通过会话管理、技能与工具系统提供可插拔扩展点。A2UI 渲染系统贯穿工具结果与 UI 组件的桥接。Studio 服务层提供纯业务逻辑支持，前端采用现代化 React 19 框架。

```mermaid
sequenceDiagram
participant Client as "客户端"
participant API as "Chat 路由(api/chat.py)"
participant Agent as "AgentRunner(core/runner.py)"
participant LLM as "LLMCaller(core/llm/caller.py)"
participant Tools as "工具系统(core/tools/base.py)"
participant Flow as "TaskFlow 框架(core/flow/*)"
participant Session as "会话管理(core/session.py)"
...
- 外部依赖
  - LLM 后端（LangChain Chat 模型）、文件系统（会话与记忆）、SSE 输出格式化。
- 循环依赖
...
## 结论
Ark-Agentic 通过"多 Agent + 统一 Runner + TaskFlow 编排 + 服务层重构 + 前端现代化"的架构，实现了高度模块化、可扩展和可维护的设计。TaskFlow 框架提供了确定性的流程状态管理，服务层重构实现了业务逻辑与接口层的解耦，前端现代化提升了开发体验和用户体验。该架构具备良好的扩展性与可维护性，适合在金融与企业服务场景中持续演进。
