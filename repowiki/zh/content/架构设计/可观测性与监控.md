# 可观测性与监控

<cite>
**本文引用的文件**
- [app.py](file://src/ark_agentic/app.py)
- [decorators.py](file://src/ark_agentic/core/observability/decorators.py)
- [tracing.py](file://src/ark_agentic/core/observability/tracing.py)
- [tracing_lifecycle.py](file://src/ark_agentic/core/observability/tracing_lifecycle.py)
- [providers/__init__.py](file://src/ark_agentic/core/observability/providers/__init__.py)
- [providers/phoenix.py](file://src/ark_agentic/core/observability/providers/phoenix.py)
- [providers/langfuse.py](file://src/ark_agentic/core/observability/providers/langfuse.py)
- [providers/console.py](file://src/ark_agentic/core/observability/providers/console.py)
- [providers/otlp.py](file://src/ark_agentic/core/observability/providers/otlp.py)
- [runner.py](file://src/ark_agentic/core/runner.py)
- [chat.py](file://src/ark_agentic/api/chat.py)
- [deps.py](file://src/ark_agentic/api/deps.py)
- [lifecycle.py](file://src/ark_agentic/core/protocol/lifecycle.py)
- [bootstrap.py](file://src/ark_agentic/core/protocol/bootstrap.py)
- [.env-sample](file://.env-sample)
- [test_tracing.py](file://tests/unit/core/test_tracing.py)
</cite>

## 更新摘要
**所做更改**
- 完全重构可观测性系统架构，从 Phoenix 回调系统迁移到基于 OpenTelemetry 的装饰器架构
- 新增 traced_agent()、traced_chain()、traced_tool() 三个核心装饰器
- 新增 add_span_attributes()、add_span_input()、add_span_output() 三个辅助函数
- 重新设计 Provider 注册表，支持多 Provider 并行导出
- 更新环境变量配置，使用 TRACING 统一控制 Provider 选择
- 移除旧的回调系统，采用装饰器模式实现更清晰的追踪边界
- **新增**：引入生命周期管理架构，`tracing.py` 重命名为 `tracing_lifecycle.py`
- **新增**：`TracingLifecycle` 类实现 `BaseLifecycle` 接口，提供统一的追踪生命周期管理
- **新增**：Bootstrap 系统自动加载 `TracingLifecycle` 作为总是运行的框架组件

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [组件详解](#组件详解)
6. [生命周期管理](#生命周期管理)
7. [依赖关系分析](#依赖关系分析)
8. [性能考量](#性能考量)
9. [故障排查指南](#故障排查指南)
10. [结论](#结论)
11. [附录](#附录)

## 简介
本文件面向重构后的可观测性与监控系统，围绕基于 OpenTelemetry 的装饰器架构和生命周期管理展开，重点说明以下方面：
- 装饰器架构设计：通过 traced_agent()、traced_chain()、traced_tool() 三个核心装饰器实现细粒度的追踪控制。
- 多 Provider 支持：统一的 Provider 注册表支持 Phoenix、Langfuse、Console、OTLP 等多种后端。
- 动态属性注入：add_span_attributes()、add_span_input()、add_span_output() 提供运行时动态属性写入。
- 环境变量驱动：TRACING 环境变量统一控制 Provider 选择和启用方式。
- 分布式追踪与链路分析：通过装饰器在执行边界创建语义化 Span，支持复杂的嵌套调用树。
- **新增**：生命周期管理：`TracingLifecycle` 类提供统一的追踪系统启动和停止管理。

## 项目结构
重构后的可观测性系统采用四层架构：业务回调层、装饰器层、Provider 注册层、生命周期管理层，形成清晰的职责分离。

```mermaid
graph TB
subgraph "应用入口"
APP["app.py<br/>FastAPI 应用"]
ENDL["生命周期管理<br/>Bootstrap"]
end
subgraph "可观测性装饰器层"
DEC["observability/decorators.py<br/>traced_agent / traced_chain / traced_tool"]
HELP["辅助函数<br/>add_span_attributes / add_span_input / add_span_output"]
ENDL --> DEC
end
subgraph "生命周期管理层"
TL["observability/tracing_lifecycle.py<br/>TracingLifecycle"]
ENDL --> TL
end
subgraph "Provider 注册层"
REG["observability/providers/__init__.py<br/>Provider 注册表"]
PHX["PhoenixProvider"]
LF["LangfuseProvider"]
CON["ConsoleProvider"]
OTLP["OTLPProvider"]
TL --> REG
end
subgraph "核心执行层"
RUNNER["core/runner.py<br/>AgentRunner 执行器"]
end
subgraph "API 层"
CHAT["api/chat.py<br/>/chat 路由"]
DEPS["api/deps.py<br/>共享依赖"]
end
APP --> DEC
DEC --> RUNNER
REG --> PHX
REG --> LF
REG --> CON
REG --> OTLP
CHAT --> DEPS
CHAT --> RUNNER
```

**图表来源**
- [app.py:93](file://src/ark_agentic/app.py#L93)
- [decorators.py:101](file://src/ark_agentic/core/observability/decorators.py#L101)
- [tracing_lifecycle.py:21](file://src/ark_agentic/core/observability/tracing_lifecycle.py#L21)
- [providers/__init__.py:30](file://src/ark_agentic/core/observability/providers/__init__.py#L30)
- [runner.py:38](file://src/ark_agentic/core/runner.py#L38)

## 核心组件
- **装饰器层**：traced_agent()、traced_chain()、traced_tool() 三个装饰器分别处理 Agent、Chain、Tool 三类执行阶段。
- **辅助函数**：add_span_attributes()、add_span_input()、add_span_output() 提供运行时动态属性写入能力。
- **Provider 注册表**：PROVIDERS 字典统一管理所有可用的追踪后端，支持自动发现和手动选择。
- **生命周期管理**：`TracingLifecycle` 类实现 `BaseLifecycle` 接口，提供统一的追踪系统启动和停止管理。
- **环境变量配置**：TRACING 环境变量控制 Provider 选择，支持逗号分隔的多 Provider 组合。
- **自动仪器化**：集成 openinference-instrumentation-langchain，自动捕获 LLM 调用细节。

**章节来源**
- [decorators.py:101](file://src/ark_agentic/core/observability/decorators.py#L101)
- [decorators.py:149](file://src/ark_agentic/core/observability/decorators.py#L149)
- [providers/__init__.py:30](file://src/ark_agentic/core/observability/providers/__init__.py#L30)
- [tracing.py:56](file://src/ark_agentic/core/observability/tracing.py#L56)
- [tracing_lifecycle.py:21](file://src/ark_agentic/core/observability/tracing_lifecycle.py#L21)

## 架构总览
重构后的架构采用装饰器模式，在执行边界精确控制追踪行为，形成清晰的调用树结构，并通过生命周期管理确保追踪系统的正确启动和停止。

```mermaid
sequenceDiagram
participant Client as "客户端"
participant API as "FastAPI /chat"
participant Deps as "API 依赖"
participant Runner as "AgentRunner"
participant Decorators as "装饰器层"
participant Lifecycle as "生命周期管理"
participant Provider as "Provider 注册表"
Client->>API : POST /chat
API->>Deps : 获取 AgentRunner
API->>Runner : run(session_id, user_input, input_context, stream?)
Runner->>Decorators : @traced_agent("agent.run")
Decorators->>Decorators : 创建 AGENT Span
Runner->>Decorators : @traced_chain("agent.turn")
Decorators->>Decorators : 创建 CHAIN Span
Runner->>Decorators : @traced_chain("agent.model_phase")
Decorators->>Decorators : 创建 CHAIN Span
Runner->>Decorators : @traced_chain("agent.tool_phase")
Decorators->>Decorators : 创建 CHAIN Span
Runner->>Decorators : @traced_tool
Decorators->>Decorators : 创建 TOOL Span
Runner-->>Decorators : 返回结果
Decorators-->>Provider : 上报所有 Span
Lifecycle-->>Provider : 生命周期管理
```

**图表来源**
- [chat.py:27](file://src/ark_agentic/api/chat.py#L27)
- [runner.py:38](file://src/ark_agentic/core/runner.py#L38)
- [decorators.py:101](file://src/ark_agentic/core/observability/decorators.py#L101)
- [tracing_lifecycle.py:32](file://src/ark_agentic/core/observability/tracing_lifecycle.py#L32)

## 组件详解

### 装饰器架构设计
重构后的装饰器系统提供三种核心装饰器，分别处理不同类型的执行阶段：

- **traced_agent()**：包装 AgentRunner.run() 方法，创建 AGENT 类型的 Span，适用于整个 Agent 执行过程的追踪。
- **traced_chain()**：包装执行循环中的各个阶段（turn、model_phase、tool_phase），创建 CHAIN 类型的 Span，表示链式调用过程。
- **traced_tool()**：包装 ToolExecutor._execute_single() 方法，创建 TOOL 类型的 Span，自动从 ToolCall.name 获取工具名称。

```mermaid
classDiagram
class TracedAgent {
+traced_agent(span_name="agent.run")
+创建 AGENT 类型 Span
+自动设置错误状态
}
class TracedChain {
+traced_chain(span_name)
+创建 CHAIN 类型 Span
+支持嵌套调用
}
class TracedTool {
+traced_tool(fn)
+创建 TOOL 类型 Span
+自动提取工具参数
+检测工具错误
}
class HelperFunctions {
+add_span_attributes(dict)
+add_span_input(value)
+add_span_output(value)
}
TracedAgent --> HelperFunctions : "使用"
TracedChain --> HelperFunctions : "使用"
TracedTool --> HelperFunctions : "使用"
```

**图表来源**
- [decorators.py:101](file://src/ark_agentic/core/observability/decorators.py#L101)
- [decorators.py:149](file://src/ark_agentic/core/observability/decorators.py#L149)

**章节来源**
- [decorators.py:78](file://src/ark_agentic/core/observability/decorators.py#L78)
- [decorators.py:101](file://src/ark_agentic/core/observability/decorators.py#L101)
- [decorators.py:111](file://src/ark_agentic/core/observability/decorators.py#L111)

### Provider 注册表与多后端支持
新的 Provider 注册表采用统一接口 TracingProvider，支持多种追踪后端的并行导出：

- **PhoenixProvider**：支持 Arize Phoenix 本地或远程收集器，自动仪器化支持。
- **LangfuseProvider**：支持 Langfuse Cloud，使用 Basic Auth 认证。
- **ConsoleProvider**：本地开发用控制台输出，便于调试。
- **OTLPProvider**：通用 OTLP 导出器，支持任意兼容的 OTel 收集器。

```mermaid
flowchart TD
Start(["进程启动"]) --> CheckEnv["读取 TRACING 环境变量"]
CheckEnv --> ParseSpec["解析 Provider 规范"]
ParseSpec --> ResolveAuto{"是否 auto 模式?"}
ResolveAuto --> |是| AutoDiscover["自动发现可用 Provider"]
ResolveAuto --> |否| ManualSelect["手动选择指定 Provider"]
AutoDiscover --> BuildTP["构建 TracerProvider"]
ManualSelect --> BuildTP
BuildTP --> InstallProviders["安装各 Provider"]
InstallProviders --> EnableLC["启用 LangChain 自动仪器化"]
EnableLC --> Ready["就绪"]
```

**图表来源**
- [tracing.py:56](file://src/ark_agentic/core/observability/tracing.py#L56)
- [providers/__init__.py:30](file://src/ark_agentic/core/observability/providers/__init__.py#L30)

**章节来源**
- [tracing.py:35](file://src/ark_agentic/core/observability/tracing.py#L35)
- [providers/__init__.py:30](file://src/ark_agentic/core/observability/providers/__init__.py#L30)

### 辅助函数与动态属性注入
装饰器系统提供三个辅助函数，支持运行时动态属性写入：

- **add_span_attributes()**：将字典形式的静态属性写入当前 Span，自动类型转换和空值过滤。
- **add_span_input()**：将输入值序列化为 JSON 写入 input.value 属性，便于链路分析。
- **add_span_output()**：将输出值序列化为 JSON 写入 output.value 属性，支持复杂对象结构。

```mermaid
sequenceDiagram
participant Decorator as "装饰器"
participant Helper as "辅助函数"
participant Span as "当前 Span"
Decorator->>Helper : add_span_attributes({"key" : "value"})
Helper->>Span : set_attribute("key", "value")
Decorator->>Helper : add_span_input(input_data)
Helper->>Span : set_attribute("input.value", json)
Decorator->>Helper : add_span_output(output_data)
Helper->>Span : set_attribute("output.value", json)
```

**图表来源**
- [decorators.py:149](file://src/ark_agentic/core/observability/decorators.py#L149)
- [decorators.py:165](file://src/ark_agentic/core/observability/decorators.py#L165)

**章节来源**
- [decorators.py:149](file://src/ark_agentic/core/observability/decorators.py#L149)
- [decorators.py:165](file://src/ark_agentic/core/observability/decorators.py#L165)

### API 集成与生命周期管理
应用启动时通过 `setup_tracing_from_env()` 初始化追踪系统，支持多 Provider 并行工作。新的生命周期管理架构确保追踪系统在整个应用生命周期中正确运行：

- **启动流程**：读取 TRACING 环境变量 → 解析 Provider 规范 → 构建 TracerProvider → 安装各 Provider → 启用 LangChain 自动仪器化。
- **关闭流程**：优雅关闭所有已安装的 Provider → 清理 TracerProvider 资源。
- **错误处理**：Provider 安装失败不影响主流程，记录警告信息继续执行。

**章节来源**
- [app.py:93](file://src/ark_agentic/app.py#L93)
- [tracing.py:56](file://src/ark_agentic/core/observability/tracing.py#L56)
- [tracing.py:102](file://src/ark_agentic/core/observability/tracing.py#L102)

## 生命周期管理

### TracingLifecycle 类设计
新的 `TracingLifecycle` 类实现了 `BaseLifecycle` 接口，提供统一的追踪系统生命周期管理：

- **自动加载**：Bootstrap 系统自动加载 `TracingLifecycle` 作为总是运行的框架组件。
- **服务名管理**：从环境变量 `OTEL_SERVICE_NAME` 读取服务名，支持 CLI/worker 主机复用。
- **启动管理**：`start()` 方法调用 `setup_tracing_from_env()` 初始化追踪系统。
- **停止管理**：`stop()` 方法调用 `shutdown_tracing()` 优雅关闭追踪系统。

```mermaid
classDiagram
class TracingLifecycle {
+name = "tracing"
+__init__(service_name)
+start(ctx) Async
+stop() Async
-_service_name : str
-_provider : Any
}
class BaseLifecycle {
+name : str
+is_enabled() bool
+init() Async
+install_routes(app) None
+start(ctx) Async
+stop() Async
}
class Bootstrap {
+components : list[Lifecycle]
+start(ctx) Async
+stop() Async
}
TracingLifecycle --|> BaseLifecycle : 实现
Bootstrap --> TracingLifecycle : 自动加载
```

**图表来源**
- [tracing_lifecycle.py:21](file://src/ark_agentic/core/observability/tracing_lifecycle.py#L21)
- [lifecycle.py:24](file://src/ark_agentic/core/protocol/lifecycle.py#L24)
- [bootstrap.py:67](file://src/ark_agentic/core/protocol/bootstrap.py#L67)

**章节来源**
- [tracing_lifecycle.py:21](file://src/ark_agentic/core/observability/tracing_lifecycle.py#L21)
- [lifecycle.py:24](file://src/ark_agentic/core/protocol/lifecycle.py#L24)
- [bootstrap.py:67](file://src/ark_agentic/core/protocol/bootstrap.py#L67)

### Bootstrap 集成
Bootstrap 系统现在自动加载 `TracingLifecycle` 作为总是运行的框架组件，确保追踪系统在整个应用生命周期中正确运行：

- **固定顺序**：`AgentsLifecycle`（第一个）和 `TracingLifecycle`（最后一个）总是加载。
- **插件间隔**：用户选择的插件位于这两个固定组件之间。
- **生命周期协调**：Bootstrap 确保 `TracingLifecycle` 在所有其他组件之后停止。

**章节来源**
- [bootstrap.py:67](file://src/ark_agentic/core/protocol/bootstrap.py#L67)
- [bootstrap.py:71](file://src/ark_agentic/core/protocol/bootstrap.py#L71)
- [bootstrap.py:73](file://src/ark_agentic/core/protocol/bootstrap.py#L73)

## 依赖关系分析
重构后的依赖关系更加清晰，采用装饰器模式替代回调系统，并引入生命周期管理：

```mermaid
graph LR
APP["app.py"] --> SETUP["setup_tracing_from_env()"]
SETUP --> TP["TracerProvider"]
TP --> REG["PROVIDERS 注册表"]
REG --> PHX["PhoenixProvider"]
REG --> LF["LangfuseProvider"]
REG --> CON["ConsoleProvider"]
REG --> OTLP["OTLPProvider"]
RUNNER["runner.py"] --> DEC["装饰器导入"]
DEC --> TRACED_AGENT["@traced_agent"]
DEC --> TRACED_CHAIN["@traced_chain"]
DEC --> TRACED_TOOL["@traced_tool"]
TRACED_AGENT --> TP
TRACED_CHAIN --> TP
TRACED_TOOL --> TP
BOOTSTRAP["bootstrap.py"] --> TL["TracingLifecycle"]
TL --> SETUP
```

**图表来源**
- [app.py:93](file://src/ark_agentic/app.py#L93)
- [runner.py:38](file://src/ark_agentic/core/runner.py#L38)
- [decorators.py:101](file://src/ark_agentic/core/observability/decorators.py#L101)
- [bootstrap.py:67](file://src/ark_agentic/core/protocol/bootstrap.py#L67)

**章节来源**
- [app.py:93](file://src/ark_agentic/app.py#L93)
- [runner.py:38](file://src/ark_agentic/core/runner.py#L38)
- [decorators.py:101](file://src/ark_agentic/core/observability/decorators.py#L101)
- [bootstrap.py:67](file://src/ark_agentic/core/protocol/bootstrap.py#L67)

## 性能考量
新架构在保持高性能的同时提供了更好的可观测性：

- **零成本降级**：未配置 TRACING 时使用 OpenTelemetry NoOp tracer，装饰器调用几乎无额外开销。
- **批量导出**：所有 Provider 使用 BatchSpanProcessor，减少网络往返开销。
- **内存友好**：装饰器只在执行边界创建 Span，避免持续的追踪开销。
- **自动仪器化**：LangChain 自动仪器化捕获 LLM 调用细节，无需手动添加追踪代码。
- **生命周期优化**：通过生命周期管理确保追踪系统正确启动和停止，避免资源泄漏。

**章节来源**
- [tracing.py:64](file://src/ark_agentic/core/observability/tracing.py#L64)
- [providers/phoenix.py:32](file://src/ark_agentic/core/observability/providers/phoenix.py#L32)
- [providers/langfuse.py:40](file://src/ark_agentic/core/observability/providers/langfuse.py#L40)
- [tracing_lifecycle.py:32](file://src/ark_agentic/core/observability/tracing_lifecycle.py#L32)

## 故障排查指南
针对新架构的常见问题和解决方案：

### 环境变量配置
- **TRACING 选择**：支持 console、phoenix、langfuse、otlp 或逗号分隔的组合，auto 模式自动启用有凭据的 Provider。
- **Phoenix 配置**：PHOENIX_COLLECTOR_ENDPOINT 必填，支持本地 6006 端口。
- **Langfuse 配置**：LANGFUSE_PUBLIC_KEY 和 LANGFUSE_SECRET_KEY 必填，LANGFUSE_HOST 可选。
- **OTLP 配置**：OTEL_EXPORTER_OTLP_ENDPOINT 必填，支持自定义头部。
- **服务名配置**：OTEL_SERVICE_NAME 环境变量用于生命周期管理。

### Provider 初始化问题
- **自动发现失败**：检查凭据环境变量是否正确设置，查看应用启动日志中的 Provider 状态。
- **安装失败**：单个 Provider 安装失败不会影响其他 Provider，查看对应 Provider 的错误日志。
- **网络连接**：确保能够访问配置的收集器端点，检查防火墙和代理设置。

### 装饰器使用问题
- **装饰器顺序**：确保装饰器按照正确的执行顺序应用，Agent 装饰器在外层，Chain 装饰器在内层。
- **属性写入**：使用 add_span_attributes() 在装饰器内部正确写入动态属性。
- **异常处理**：装饰器自动处理异常并设置 Span 状态，检查异常类型和消息。

### 生命周期管理问题
- **启动失败**：检查 TRACING 环境变量配置，确认 Provider 凭据正确设置。
- **停止失败**：查看生命周期管理日志，确认所有 Provider 正确关闭。
- **资源泄漏**：确保应用正常关闭，生命周期管理会自动清理追踪资源。

**章节来源**
- [.env-sample:35](file://.env-sample#L35)
- [providers/phoenix.py:18](file://src/ark_agentic/core/observability/providers/phoenix.py#L18)
- [providers/langfuse.py:19](file://src/ark_agentic/core/observability/providers/langfuse.py#L19)
- [test_tracing.py:127](file://tests/unit/core/test_tracing.py#L127)
- [tracing_lifecycle.py:32](file://src/ark_agentic/core/observability/tracing_lifecycle.py#L32)

## 结论
重构后的可观测性系统以装饰器架构为核心，结合生命周期管理，提供了更清晰、更灵活、更可靠的追踪能力：

- **明确的执行边界**：通过 traced_agent()、traced_chain()、traced_tool() 精确定位执行阶段。
- **统一的 Provider 管理**：PROVIDERS 注册表支持多后端并行导出，便于混合部署。
- **动态属性注入**：add_span_* 辅助函数支持运行时动态属性写入，适应复杂业务场景。
- **零成本降级**：未配置时自动降级为 NoOp tracer，不影响生产性能。
- **自动仪器化**：LangChain 自动仪器化捕获 LLM 调用细节，提供完整的端到端追踪。
- **生命周期管理**：`TracingLifecycle` 类确保追踪系统在整个应用生命周期中正确运行，避免资源泄漏。

建议在生产环境中：
- 使用 TRACING=auto 自动发现可用的 Provider，或明确指定需要的后端。
- 结合 Provider UI 建立关键指标看板，重点关注错误率、延迟分布和资源使用情况。
- 基于装饰器生成的语义化属性建立告警规则，实现精准的问题定位。
- 监控生命周期管理日志，确保追踪系统正确启动和停止。

## 附录
- **环境变量清单**
  - TRACING：控制启用的 Provider，支持逗号分隔的多个值或 auto 模式
  - PHOENIX_COLLECTOR_ENDPOINT：Phoenix 收集器端点地址
  - LANGFUSE_PUBLIC_KEY/SECRET_KEY：Langfuse 认证凭据
  - LANGFUSE_HOST：Langfuse 服务主机地址
  - OTEL_EXPORTER_OTLP_ENDPOINT/HEADERS：OTLP 导出器配置
  - OTEL_SERVICE_NAME：服务名称，用于生命周期管理
- **装饰器使用示例**
  - @traced_agent("agent.run")：包装 AgentRunner.run()
  - @traced_chain("agent.turn")：包装执行循环阶段
  - @traced_tool：包装工具执行方法
- **辅助函数**
  - add_span_attributes()：写入静态属性
  - add_span_input()：写入输入数据
  - add_span_output()：写入输出数据
- **生命周期管理**
  - TracingLifecycle：统一的追踪系统生命周期管理
  - Bootstrap：自动加载 TracingLifecycle 作为总是运行的组件

**章节来源**
- [.env-sample:35](file://.env-sample#L35)
- [decorators.py:101](file://src/ark_agentic/core/observability/decorators.py#L101)
- [decorators.py:149](file://src/ark_agentic/core/observability/decorators.py#L149)
- [tracing_lifecycle.py:21](file://src/ark_agentic/core/observability/tracing_lifecycle.py#L21)
- [bootstrap.py:67](file://src/ark_agentic/core/protocol/bootstrap.py#L67)