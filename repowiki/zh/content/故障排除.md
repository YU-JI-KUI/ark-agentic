...
## 更新摘要
**变更内容**
- 新增维基系统章节，介绍repowiki目录结构和多语言文档系统
- 更新应用入口章节，包含维基API端点和文档树构建
- 增强故障排除指南，涵盖维基系统相关的常见问题
- 添加维基系统调试工具和配置选项
...
**新增** 本版本特别增加了对全新维基系统的故障排除指导，该系统提供了完整的英文和中文文档支持，包含故障排除、FAQ、常见问题解答等内容。
...
- 应用入口：app
- **新增** 维基系统：repowiki目录结构，支持中英文双语文档
...
- **新增** 维基系统：完整的 repowiki 目录结构，支持中英文双语文档，提供故障排除、FAQ、常见问题解答等内容。
...
- **新增** 维基系统依赖 FastAPI 路由，提供 /api/wiki/tree 和 /api/wiki/{lang}/{path} 端点。
...
APP --> TR
APP --> WIKI["维基系统API"]
WIKI --> TREE["/api/wiki/tree"]
WIKI --> PAGE["/api/wiki/{lang}/{path}"]
```
...
- **新增** 维基系统性能：文档树构建使用元数据排序，确保文档加载顺序的一致性和性能优化。
...
### 九、维基系统概述
Ark-Agent 现已集成了完整的维基系统（repowiki），提供中英文双语文档支持，包含故障排除、FAQ、常见问题解答等内容。该系统通过 FastAPI 端点提供动态文档加载和导航。

### 十、维基系统架构
- 目录结构：repowiki/zh/content/ 和 repowiki/en/content/
- 元数据管理：repowiki-metadata.json 控制文档目录顺序
- API 端点：/api/wiki/tree（文档树）、/api/wiki/{lang}/{path}（文档内容）
- 前端集成：home.html 中的 Wiki 标签页和语言切换功能
...
ZH["zh/content/"]
EN["en/content/"]
TREEAPI["/api/wiki/tree"]
PAGEAPI["/api/wiki/{lang}/{path}"]
HOMEHTML["home.html Wiki集成"]
end
WIKIROOT --> META
WIKIROOT --> ZH
WIKIROOT --> EN
...
- [src/ark_agentic/app.py:198-263](file://src/ark_agentic/app.py#L198-L263)
- [src/ark_agentic/static/home.html:555-620](file://src/ark_agentic/static/home.html#L555-L620)
...
- 文档树加载失败
  - 现象：Wiki 标签页显示"加载失败"
  - 诊断：检查 /api/wiki/tree 端点响应
  - 修复：确认 repowiki 目录结构和元数据文件
- 文档内容无法显示
  - 现象：选择文档后显示"加载失败"
  - 诊断：检查 /api/wiki/{lang}/{path} 端点
  - 修复：验证文档路径和文件权限
- 语言切换问题
  - 现象：中文/英文按钮无响应
  - 诊断：检查前端 JavaScript 事件绑定
  - 修复：重新加载页面或检查浏览器控制台

#### 维基系统调试方法
- API 端点测试
  - GET /api/wiki/tree：验证文档树结构
  - GET /api/wiki/zh/{path}：验证中文文档内容
  - GET /api/wiki/en/{path}：验证英文文档内容
- 日志分析
  - 检查 FastAPI 应用日志中的维基系统错误
  - 关注路径解析和文件访问相关的异常
- 前端调试
  - 使用浏览器开发者工具检查网络请求
  - 验证 Markdown 渲染和 Mermaid 图表显示

#### 维基系统配置与部署
- 目录要求
  - 必须存在 repowiki/zh/content/ 和 repowiki/en/content/
  - 每个语言目录必须包含有效的 Markdown 文件
- 元数据配置
  - repowiki-metadata.json 必须包含 wiki_catalogs 和 wiki_items
  - wiki_items 中的目录 ID 必须与实际目录结构匹配
- 安全考虑
  - 路径穿越保护：系统自动验证文件路径安全性
  - 权限控制：确保文档文件具有适当的读取权限
...
### 十二、维基系统与故障排除的关系
- 故障排除文档：维基系统包含专门的故障排除章节，提供系统化的问题诊断方法
- FAQ 集成：常见问题解答通过维基系统提供多语言支持
- 实时更新：维基系统允许快速更新故障排除指南和最佳实践
- 多语言支持：中英文双语文档确保全球用户都能获得帮助
...
## 结论
通过统一的 LLM 错误分类、可插拔的重试策略、完善的回调与追踪体系，以及清晰的流式事件模型，Ark-Agent 能够在复杂生产环境中快速定位问题、稳定运行并持续优化。**新增的维基系统进一步增强了系统的可维护性和用户支持能力**，通过完整的中英文文档体系、故障排除指南和 FAQ 集成，为用户提供全面的技术支持。

建议在生产中启用可观测性、建立基于错误原因与指标的告警，并配合本指南的排查清单与流程，实现高效的问题闭环。同时，充分利用维基系统的故障排除功能，定期更新文档内容，确保用户能够及时获得最新的技术支持。

## 附录
- 应用入口与生命周期
  - app.py 在 lifespan 中初始化 tracing、注册 Agent、warmup 并在关闭时优雅停机。
  - **新增** app.py 集成了完整的维基系统 API 端点，支持文档树构建和内容加载。
- 事件模型参考
  - AG-UI 事件类型与字段定义，便于前端对齐与调试。
- **新增** 维基系统 API 参考
  - /api/wiki/tree：返回 repowiki 两种语言的目录树
